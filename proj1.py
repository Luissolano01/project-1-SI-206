...
def pct_calc(data_dict):
    ...
    for item in data_dict:
        ...
        updated_nested_dict = {}   # new empty nested dictionary
        nested_dict = data_dict[item]  # passes in groups
        
        for column in nested_dict:  # iterates through each column
            if column != "State Totals":  # makes sure "State Totals" is not included
                value = (nested_dict[column]/nested_dict["State Totals"])
                rounded_percent = round((value * 100), 2)
                updated_nested_dict[column] = rounded_percent
        pct_dict[item] = updated_nested_dict
...
def pct_dif(data_dict1, data_dict2):
    ...
    for state in data_dict1: # iterates through each state in outer dict
        ...
        state2 = state.replace("-"," ")   # takes care of columns with dashes in name
        for column in data_dict1[state]:   #iterate through inner dict for both 1st and 2nd dict
            ...
            if column != "State Totals" and column != "NO RESPONSE": 
                ...
                second_value = data_dict2[state2][column] #to get value in 2nd dict           !!!!!!!!!!!!!!!!!!!PROGRESS
...
def max_min(data_dict):
    ...
    for ethnicity in data_dict.keys():
        max_min["max"].setdefault(ethnicity, {})    #passing ethnicities to max dict 
        max_min["min"].setdefault(ethnicity, {})    #passing ethnicities to min dict

        #maximum_demographic.setdefault(ethnicity, {})     
        #minimum_demographic.setdefault(ethnicity, {})     
        for group in max_min["min"]:
            minimum_list = sorted(data_dict[group].items(), key = lambda x: x[1])
            five_min = minimum_list[:5]
            minimum = dict(five_min)
        minimum_demographic[group] = minimum
